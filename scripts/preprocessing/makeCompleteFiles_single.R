...
for (expVersion in list.files(path = path)) {
  ...
  for (ppt in list.files(path = paste(path, expVersion, sep = "/"))) {
    ...
    for (session in list.files(path = paste(path,
      expVersion, ppt,
      sep = "/"
    ))) {
      dir.create(paste(path,
        "complete", expVersion, ppt, session,
        sep = "/"
      ))
      ...
      for (trackerTag in c("cursorobjecttracker", "trackerholderobject")) {
        ...
        fwrite(complete_df, file = paste(path, "complete",
          expVersion, ppt, session, fileName,
          sep = "/"
        ))
      }

      # copy over the trial results file
      trialResultPath <- list.files(
        path = paste(path, expVersion, ppt, session, sep = "/"),
      ...
      fwrite(trialResult, file = paste(path, "complete",
        expVersion, ppt, session, "trial_results.csv",
        sep = "/"
      ))
    }
  }
